...
class StatsLogger:
    ...
    def __init__(
        ...
        save_summary : bool, optional
            Whether to write TensorBoard summaries.
        ...
        self.all_r = (
            []
        )  # save all R separately to keep backward compatibility with a generated file.
        ...
        self.buffer_all_programs = (
            StringIO()
        )  # Buffer for the statistics for all programs.
    ...
    def save_stats(
        ...
        if self.output_file is not None:
            ...
            a_ent_full = np.mean(
                np.apply_along_axis(empirical_entropy, 0, actions_full)
            )
        ...
        if self.save_all_iterations:
            all_iteration_stats = np.array(
                [[iteration] * len(r_full), r_full, l_full, invalid_full]
            ).transpose()
        ...
        if self.save_top_samples_per_batch > 0:
            df_topsamples = pd.DataFrame(
                top_samples_per_batch, columns=["Iteration", "Reward", "Sequence"]
            )
            # only save to the file the current iteration
            header = iteration == 1
            df_topsamples.to_csv(
                self.buffer_top_samples, mode="a", index=False, header=header
            )
    ...
    def save_results(self, pool, n_samples):
        ...
        if n_invalid > 0:
            print(
                "Invalid expressions: {} of {} ({:.1%}).".format(
                    n_invalid, n_samples, n_invalid / n_samples
                )
            )
