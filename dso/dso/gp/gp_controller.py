...
class GPController:
    ...
    def __init__(
        ...
        **kwargs,
        ...
        assert (
            crossover_operator in crossover_ops_dict.keys()
        ), f"Crossover operator should be one of the following: {list(crossover_ops_dict.keys())}"
        self.crossover_operator = crossover_operator

        assert (
            mutation_operator in mutation_ops_dict.keys()
        ), f"Mutation operator should be one of the following: {list(mutation_ops_dict.keys())}"
        self.mutation_operator = mutation_operator
        ...
        self.algorithm = gp_base.RunOneStepAlgorithm(
            toolbox=self.toolbox, p_crossover=p_crossover, verbose=verbose
        )

    def check_constraint(self, individual):
        actions, parents, siblings = U.individual_to_dso_aps(
            individual, Program.library
        )
        return self.prior.is_violated(actions, parents, siblings)

    def _create_toolbox(
        self, pset, tournament_size=3, mutate_tree_max=5, parallel_eval=True
    ):
        """
        ...
        toolbox.register(
            "mutate", mut_op, expr=toolbox.expr_mut, pset=pset, indpb=self.p_mutate
        )
        ...
        toolbox.decorate(
            "mutate", U.staticLimit(key=self.check_constraint, max_value=0)
        )
    ...
    def get_hof_programs(self):
        ...
        self.max_length = max(max(len(ind) for ind in hof), self.max_length)
    ...
    def __call__(self, actions):
        ...
        if self.population_size < actions.shape[0]:
            samples_idx = np.random.choice(
                actions.shape[0], self.population_size, replace=False
            )
        else:
    ...
    def verbose_print(self, timer, programs):
        ...
        print("--------------------------------------------------")
        self._extracted_from_verbose_print_11("GP -> best program this iteration:")
        print(programs[0].print_stats())
        self._extracted_from_verbose_print_11(
            "--------------------------------------------------"
        )

    # TODO Rename this here and in `verbose_print`
    def _extracted_from_verbose_print_11(self, arg0):
        print()
        print(arg0)
        print()
