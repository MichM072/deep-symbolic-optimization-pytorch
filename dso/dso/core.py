...
class DeepSymbolicOptimizer:
    ...
    def finish(self):
        ...
        result = {"seed": self.config_experiment["seed"], "r": p.r}
        result.update(p.evaluate)
        result.update(
            {"expression": repr(p.sympy_expr), "traversal": repr(p), "program": p}
        )
    ...
    def make_logger(self):
        return StatsLogger(self.output_file, **self.config_logger)

    def make_checkpoint(self):
        return Checkpoint(self, **self.config_checkpoint)

    def make_policy_optimizer(self):
        return make_policy_optimizer(self.policy, **self.config_policy_optimizer)
    ...
    def make_output_file(self):
        ...
        if self.config_experiment["exp_name"] is None:
            ...
            save_path = os.path.join(
                self.config_experiment["logdir"], self.config_experiment["exp_name"]
            )
        ...
        output_file = os.path.join(save_path, f"dso_{task_name}_{seed}.csv")
